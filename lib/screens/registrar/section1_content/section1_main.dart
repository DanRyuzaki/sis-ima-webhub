...
import 'package:sis_project/models/registrarAnalyticsModel.dart';
import 'package:sis_project/screens/registrar/section1_content/section1_viewpub.dart';
...
class RegistrarFirstSection extends StatefulWidget {
  const RegistrarFirstSection({super.key});

  @override
  State<RegistrarFirstSection> createState() => _RegistrarFirstSectionState();
}

class _RegistrarFirstSectionState extends State<RegistrarFirstSection> {
  List<PubModel> _pubDataFetch = [];
  List<PubModel> _pubDataDeployed = [];
  late registrarAnalyticsModel _analyticsModel;
  ...
  void _initializeData() {
    ...
    _analyticsModel = registrarAnalyticsModel(
      totalClasses: 0,
      totalSubjects: 0,
      totalTeachers: 0,
      totalStudents: 0,
    );
    _fetchRegistrarAnalytics();
    _fetchPubList();
  }
  ...
  Future<void> _fetchRegistrarAnalytics() async {
    try {
      ...
      setState(() {
        _analyticsModel = registrarAnalyticsModel(
          ...
          totalSubjects: results[1],
          totalTeachers: results[2],
          totalStudents: results[3],
        );
      });
  ...
  Future<int> _getTotalClasses() async {
    ...
    for (final dept in departments) {
      final aggregate =
          await FirebaseFirestore.instance.collection(dept).count().get();
      total += aggregate.count!;
    }
    return total;
  }

  Future<int> _getTotalSubjects() async {
    final snapshot =
        await FirebaseFirestore.instance.collection("subjects").get();
    return snapshot.docs.length;
  }

  Future<int> _getTotalFaculty() async {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Welcome back, Registrar ${globalState.userName00} ${globalState.userName01}!',
            style: GoogleFonts.montserrat(
              fontSize:
      ...
      children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          _buildAnalyticsCard(
              ...
              _analyticsModel.totalClasses,
              'REGISTERED CLASSES',
              'As of ${_formatTimestamp(_timeNow, "MMMM d, yyyy | EEEE")}',
              context),
          _buildAnalyticsCard(
              HugeIcons.strokeRoundedBook02,
              _analyticsModel.totalSubjects,
              'REGISTERED SUBJECTS',
              'As of ${_formatTimestamp(_timeNow, "MMMM d, yyyy | EEEE")}',
              context),
        ]),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          _buildAnalyticsCard(
              ...
              _analyticsModel.totalTeachers,
              'EMPLOYED FACULTIES',
              'As of ${_formatTimestamp(_timeNow, "MMMM d, yyyy | EEEE")}',
              context),
          _buildAnalyticsCard(
              HugeIcons.strokeRoundedStudentCard,
              _analyticsModel.totalStudents,
              'ENROLLED STUDENTS',
              'As of ${_formatTimestamp(_timeNow, "MMMM d, yyyy | EEEE")}',
              context),
        ]),

...
import 'package:sis_project/screens/faculty/section4_content/section3_assesstudent.dart';
import 'package:sis_project/services/dynamicsize_service.dart';
import 'package:sis_project/services/global_state.dart';
import 'package:provider/provider.dart';
import 'package:google_fonts/google_fonts.dart';

class FacultyFourthSection extends StatefulWidget {
  const FacultyFourthSection({super.key});

  @override
  State<FacultyFourthSection> createState() => _FacultyFourthSectionState();
}

class _FacultyFourthSectionState extends State<FacultyFourthSection> {
  bool isSubjectStudentsListLoaded = false, isHeaderClicked = false;
  String query = '';
  double sortBy = -1;
  List<studentModel> subjectStudentsFetch = [], subjectStudentsDeployed = [];
  ...
  void initState() {
    super.initState();
    final queryClass = Uri.base.queryParameters['class'] ?? '';
    if (queryClass.isNotEmpty) {
      _searchController.text = queryClass;
      query = queryClass;
    }
  ...
  Future<void> _fetchSubjectStudents() async {
    try {
      ...
      for (var doc in userDocQS.docs) {
        final docData = doc.data();

        final advisoryClassIds = docData['advisoryClassId'];
        if (advisoryClassIds is List) {
          for (var advisoryId in advisoryClassIds) {
            if (advisoryId is String) {
              await _processClassData(advisoryId);
            }
          }
        }

        final subjectsList = docData['subjectsList'];
        if (subjectsList is List) {
          for (var subject in subjectsList) {
            if (subject is String) {
              await _processClassData(subject);
            }
          }
        }
      }
      ...
      if (mounted) {
        setState(() {
          isSubjectStudentsListLoaded = true;

          subjectStudentsDeployed = __filteredSubjectStudents(query);
        });
      }
    } catch (e) {
      print(e);
  ...
  Future<void> _processClassData(String classIdentifier) async {
    try {
      final parts = classIdentifier.split(':');
      final classPart = parts[0];
      final subjectPart = parts.length > 1 ? parts[1] : '';

      final classParts = classPart.split('-');
      if (classParts.length >= 3) {
        final classDept = classParts[0];
        final classLevel = classParts[1];
        final classSection = classParts[2];

        final studentIds = await _getStudentList(classDept, classPart);

        for (var studentId in studentIds) {
          if (studentId != 'N/A') {
            final uniqueKey = subjectPart.isNotEmpty
                ? '${studentId}_${subjectPart}'
                : studentId;

            bool studentExists = subjectStudentsFetch.any((existingStudent) =>
                subjectPart.isNotEmpty
                    ? '${existingStudent.studentSID}_${subjectPart}' ==
                        uniqueKey
                    : existingStudent.studentSID == studentId);

            if (!studentExists) {
              final studentSID = await _IDOfStudent(studentId);
              final studentName = await _nameOfStudent(studentId);
              final studentProgram = await _programOfStudent(classIdentifier);

              subjectStudentsFetch.add(
                studentModel(
                  studentSID: studentSID,
                  studentName: studentName,
                  studentClassCode: classPart,
      ...
      print('Error processing class data for $classIdentifier: $e');
    }
  }
  ...
  List<studentModel> __filteredSubjectStudents(String query) {
    final filteredStudents = query.isEmpty
        ? subjectStudentsFetch
        : subjectStudentsFetch.where((_students) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'View and manage your teaching classes academic performance.',
            style: GoogleFonts.montserrat(
              fontSize:
  ...
  Future<void> _refreshStudentsList() async {
    setState(() {
      subjectStudentsFetch.clear();
      subjectStudentsDeployed.clear();
      isSubjectStudentsListLoaded = false;
    });
    await _fetchSubjectStudents();
  }
  ...
  Widget _buildClassList() {
    final currentList = subjectStudentsDeployed;

    final sourceList = subjectStudentsFetch;
  ...
  Widget build(BuildContext context) {
    final isCurrentTabLoaded = isSubjectStudentsListLoaded;
  ...
  void _onRowTapped(studentModel student) async {
    try {
      ...
      final facultyID = Provider.of<GlobalState>(context, listen: false).userID;

      showDialog(
        context: context,
        builder: (context) => AssessStudentDialog(
          studentId: student.studentSID,
          studentName: student.studentName,
          classCode: student.studentClassCode,
          teacherId: facultyID,
          departmentCode: student.studentProgram,
        ),
      );
    } catch (e) {

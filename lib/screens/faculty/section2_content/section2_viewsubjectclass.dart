...
class ViewSubjectClassDialog extends StatefulWidget {
  final String departmentCode;
  final String classCode;
  final String subjectId;
  final String teacherId;

  const ViewSubjectClassDialog({
    Key? key,
  ...
  @override
  _ViewSubjectClassDialogState createState() => _ViewSubjectClassDialogState();
}

class _ViewSubjectClassDialogState extends State<ViewSubjectClassDialog> {
  ...
  Future<void> _fetchSubjectData() async {
    try {
      String classSubjectCode =
          '${widget.classCode}:${widget.subjectId.split('-')[1]}';
    ...
    return const Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 16),
          Text('Loading subject information...'),
        ],
      ),
    );
  }
  ...
  Widget _buildClassContent() {
    if (_classData == null || _subjectData == null) {
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  _subjectData?['subjectName'] ?? 'Subject',
                  style: GoogleFonts.montserrat(
                    fontSize: 24,
                ...
                Text(
                  '${widget.classCode} - Subject Information',
                  style: GoogleFonts.montserrat(
                    fontSize: 14,
    ...
    return _buildInfoCard(
      ...
      Column(
        ...
        children: [
          ...
          _buildDetailRow('Class Adviser', _adviserName),
  ...
  Widget _buildSubjectDetailsCard() {
    return _buildInfoCard(
      ...
      Column(
        ...
        children: [
          _buildDetailRow(
              'Subject Name', _subjectData?['subjectName'] ?? 'N/A'),
          _buildDetailRow('Subject ID', _subjectData?['subjectId'] ?? 'N/A'),
          if (_subjectData?['subjectDescription']?.isNotEmpty == true)
            _buildDetailRow('Description', _subjectData!['subjectDescription']),
          _buildDetailRow(
              'Schedule', _subjectData?['classSchedule'] ?? 'No Schedule'),
        ],
      ),
  ...
  Widget _buildStudentListCard() {
    return _buildInfoCard(
      'Students in This Class (${_studentDetails.length})',
      Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
